...
const ImageWithGlow = ({ src, alt, caption }: ImageWithGlowProps) => (
  ...
  >
    <motion.img
      src={src}
      alt={alt}
      className="w-full mx-auto rounded-xl shadow-xl border theme-border-accent-important"
...
const ProcessStep = ({ number, title, description }: ProcessStepProps) => (
  ...
  >
    <div className="flex-shrink-0">
      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-primary-500 to-primary-300 flex items-center justify-center text-base-inverse font-bold text-sm md:text-base">
        {number}
      </div>
    </div>
    <div>
    ...
    <>
      <Helmet>
        ...
        <script type="application/ld+json">
          {` { "@context": "https://schema.org", "@type": "SoftwareApplication", "name": "MangaAI - Traducteur Automatique de Manga", "url": "https://cia.ift.ulaval.ca/mangaai", "description": "Traducteur automatique de manga par intelligence artificielle avec détection de texte et traduction contextuelle", "applicationCategory": "MultimediaApplication", "operatingSystem": "Web", "author": { "@type": "Organization", "name": "Club Intelligence Artificielle - Université Laval", "url": "https://cia.ift.ulaval.ca", "logo": "https://cia.ift.ulaval.ca/banner/CIA_LOGO.webp", "description": "Club étudiant d'intelligence artificielle de l'Université Laval", "foundingLocation": { "@type": "Place", "name": "Québec, Canada" }, "parentOrganization": { "@type": "EducationalOrganization", "name": "Université Laval" }, "sameAs": [ "https://www.instagram.com/ciaulaval/", "https://www.linkedin.com/company/cia-ulaval/posts/?feedView=all", "https://github.com/cia-ulaval", "https://www.facebook.com/people/Club-dintelligence-artificielle-de-lUniversité-Laval/100089798911416/?rdid=lgzUe6mitaRXBT9H&share_url=https%3A%2F%2Fwww.facebook.com%2Fshare%2F1AqQ3bCSQp" ] }, "creator": [ { "@type": "Person", "name": "Théophile Berteloot" }, { "@type": "Person", "name": "Loïc Baret" }, { "@type": "Person", "name": "Xavier Legault" }, { "@type": "Person", "name": "Louis-Jacob Lebel" }, { "@type": "Person", "name": "John-William Lebel" } ], "featureList": [ "Détection automatique de texte dans les mangas", "Traduction contextuelle multilingue", "Génération automatique de bulles de dialogue", "Interface temps réel", "Support multiple langues" ], "screenshot": "https://cia.ift.ulaval.ca/project/mangaai.webp", "image": "https://cia.ift.ulaval.ca/project/mangaai2.webp" } `}
        </script>
      </Helmet>
      <section className="relative overflow-hidden">
        ...
        >
          ...
          <div className="text-center mb-12 md:mb-16">
            ...
            <div className="flex flex-wrap justify-center gap-2 md:gap-3 mb-6 md:mb-8">
              {features.map((feature, index) => (
                <motion.div
                  key={index}
                  className="flex items-center gap-1 md:gap-2 px-3 md:px-4 py-1 md:py-2 theme-chip-inactive rounded-full theme-accent-border"
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.4, delay: 0.4 + index * 0.1 }}
                >
                  {feature.icon}
                  <span className="text-xs md:text-sm font-medium text-accent-300 whitespace-nowrap">
                    {feature.title}
                  </span>
                </motion.div>
              ))}
            ...
            <motion.p
              className="text-base md:text-xl theme-text-secondary !text-accent-300 px-4 md:px-6 max-w-3xl mx-auto"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          <section id="team" className="mb-12 md:mb-16 relative">
            ...
            <div className="absolute top-0 right-0 w-32 md:w-64 h-32 md:h-64 bg-primary-500/5 rounded-full blur-3xl -z-10"></div>
            <div className="absolute bottom-0 left-0 w-48 md:w-96 h-48 md:h-96 bg-primary-500/5 rounded-full blur-3xl -z-10"></div>
            <motion.h2
              className="text-3xl md:text-4xl font-bold theme-text-gradient text-center mb-4 md:mb-6"
