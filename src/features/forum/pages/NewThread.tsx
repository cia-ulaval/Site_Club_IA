...
import { MarkdownRenderer, ToastContainer, useToast } from "../components";
...
export function NewThread() {
  ...
  const onSubmit = async (data: NewThreadForm) => {
    // Mode développement - créer un utilisateur factice si pas connecté
    let authorId = currentUser?.id;

    if (!authorId && import.meta.env.VITE_DEV_MODE === "true") {
      // En mode développement, utiliser un ID d'utilisateur factice
      authorId = "dev-user-" + Math.random().toString(36).substr(2, 9);
      console.log("Mode dev: utilisation d'un utilisateur factice:", authorId);
    }
    ...
    if (!authorId) {
      addToast({
        ...
        message: "Vous devez être connecté pour créer un sujet.",
      });
      return;
    }
    ...
    try {
      ...
      addToast({
        ...
        message: `Votre sujet "${data.title}" a été publié.`,
      });

      // Attendre un peu pour que l'utilisateur voie la notification
      setTimeout(() => {
        // Redirection vers la page du forum avec le nouveau sujet
        navigate("/forum", {
          replace: true,
          state: { newThreadId: newThread.id },
        });
      }, 1500);
    } catch (error: any) {
